...
public class StirlingGeneratorRenderer extends BaseTorqueRenderer<TileStirlingGenerator> {
    ...
    public static final ModelResourceLocation MODEL_LOCATION = new ModelResourceLocation(new ResourceLocation(AncientWarfareCore.MOD_ID + ":automation/stirling_generator"), "normal");
    ...
    protected void transformMovingParts(Collection<CCModel> transformedGroups, Direction frontFacing, float[] rotations, LegacyModelState state) {
        float rotation = rotations[frontFacing.get3DDataValue()];
    ...
    private void calculatePistonPosition1(float crankAngleRadians, float radius, float length) {
        float cA = Mth.cos(crankAngleRadians);
        float sA = Mth.sin(crankAngleRadians);
        float pistonPos = radius * cA + Mth.sqrt(length * length - radius * radius * sA * sA);
    ...
    private void calculatePistonPosition2(float crankAngleRadians, float radius, float length) {
        float cA = Mth.cos(crankAngleRadians);
        float sA = Mth.sin(crankAngleRadians);
        float pistonPos2 = radius * cA + Mth.sqrt(length * length - radius * radius * sA * sA);
