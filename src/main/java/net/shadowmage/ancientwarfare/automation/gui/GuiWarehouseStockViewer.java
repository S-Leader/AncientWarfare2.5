...
public class GuiWarehouseStockViewer extends GuiContainerBase<ContainerWarehouseStockViewer> {
    ...
    public void setupElements() {
        ...
        for (WarehouseStockFilter filter : getContainer().filters) {
            ...
            label = new Label(178 - 16 - 12 - 2 - Minecraft.getInstance().font.width(text), totalHeight + 4, String.valueOf(filter.getQuantity()));
            area.addGuiElement(label);

            text = filter.getFilterItem().isEmpty() ? "Empty Filter" : filter.getFilterItem().getHoverName().getString();
