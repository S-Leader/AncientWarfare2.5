...
public class GuiWarehouseControl extends GuiContainerBase<ContainerWarehouseControl> {
    ...
    public void initElements() {
        ...
        Checkbox sortOrderBox = new Checkbox(8 + 55 + 55 + 4, 6, 16, 16, "guistrings.automation.descending") {
            ...
            public void onToggled() {
                ...
                label = I18n.get(TRANSL_GUI_AUTOMATION + name);
                refreshGui();
            }
        };
        ...
        Listener l = new Listener(Listener.MOUSE_DOWN) {
            ...
            public boolean onEvent(GuiElement widget, ActivationEvent evt) {
                if (evt.mButton == 0 && widget.isMouseOverElement(evt.mx, evt.my) && !area.isMouseOverSubElement(evt.mx, evt.my)) {
                    getContainer().handleClientRequestSpecific(null, 0, hasShiftDown(), false);
                }
                return true;
            }
        };
        ...
        Button b = new Button(8, 240 - 8 - 12, 40, 12, "guistrings.automation.adjust_bounds") {
            @Override
            protected void onPressed() {
                AWMenuTypes.open(player, NetworkHandler.GUI_WORKSITE_BOUNDS, getContainer().tileEntity.getBlockPos());
            }
        };
        addGuiElement(b);

        storedLabel = new Label(8 + 40 + 4, 240 - 8 - 11, I18n.get("guistrings.warehouse.storage", getContainer().currentStored, getContainer().maxStorage));
    ...
    public void setupElements() {
        ...
        storedLabel.setText(I18n.get("guistrings.warehouse.storage", getContainer().currentStored, getContainer().maxStorage));
        ...
        for (ItemHashEntry entry : displayEntries) {
            ...
            ItemSlot slot = new ItemSlot(4 + x * 18, 4 + y * 18, displayStack, this) {
                ...
                public void onSlotClicked(ItemStack stack, boolean rightClicked) {
                    getContainer().handleClientRequestSpecific(entry.writeToNBT(), count, hasShiftDown(), rightClicked);
                }
            };
