...
public class VehicleTypeCannon extends VehicleType {
    ...
    public class CannonVarHelper extends VehicleFiringVarsHelper {
        ...
        public void onFiringUpdate() {
            if (firingTicks == 0 && !vehicle.level().isClientSide) {
                vehicle.playSound(SoundEvents.FIREWORK_ROCKET_LAUNCH, 0.50F, .25F);
                vehicle.playSound(SoundEvents.TNT_PRIMED, 1.0F, 0.5F);
            }
            firingTicks++;
            if (vehicle.level().isClientSide) {
                Vec3 muzzle = vehicle.position().add(vehicle.getMissileOffset());
                vehicle.level().addParticle(ParticleTypes.SMOKE, muzzle.x, muzzle.y, muzzle.z, 0.0D, 0.05D, 0.0D);
            }
            if (firingTicks > 10) {
                if (!vehicle.level().isClientSide) {
                    vehicle.playSound(SoundEvents.GENERIC_EXPLODE, 1.f, 1.f);
                }
        ...
        private void emitMuzzleBurst() {
            Vec3 muzzle = vehicle.position().add(vehicle.getMissileOffset());
            double horizontal = Trig.sinDegrees(vehicle.localTurretPitch + vehicle.getXRot());
            double speedX = -Trig.sinDegrees(vehicle.localTurretRotation) * horizontal * 0.18D;
            double speedY = Trig.cosDegrees(vehicle.localTurretPitch + vehicle.getXRot()) * 0.18D;
            double speedZ = -Trig.cosDegrees(vehicle.localTurretRotation) * horizontal * 0.18D;
            for (int i = 0; i < 12; i++) {
                double spreadX = vehicle.level().random.nextGaussian() * 0.025D;
                double spreadY = vehicle.level().random.nextGaussian() * 0.025D;
                double spreadZ = vehicle.level().random.nextGaussian() * 0.025D;
                vehicle.level().addParticle(i < 3 ? ParticleTypes.FLAME : ParticleTypes.POOF, muzzle.x, muzzle.y, muzzle.z,
                        speedX + spreadX, speedY + spreadY, speedZ + spreadZ);
            }
        }
