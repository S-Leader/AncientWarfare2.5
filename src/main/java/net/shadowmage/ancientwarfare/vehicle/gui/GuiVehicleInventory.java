...
public class GuiVehicleInventory extends GuiContainerBase<ContainerVehicleInventory> {
    ...
    public void initElements() {
        if (getContainer().vehicle.inventory.storageInventory.getSlots() > 0) {
            addGuiElement(new Label(8, getContainer().storageY - 10, "gui.ancientwarfarevehicle.inventory.storage"));
        }
        addGuiElement(new Label(8, getContainer().extrasY - 10, "gui.ancientwarfarevehicle.inventory.ammo"));
        addGuiElement(new Label(8 + 3 * 18 + 4, getContainer().extrasY - 10, "gui.ancientwarfarevehicle.inventory.upgrades"));
        addGuiElement(new Label(8 + 6 * 18 + 8, getContainer().extrasY - 10, "gui.ancientwarfarevehicle.inventory.armor"));
        addGuiElement(new Label(8, getContainer().playerY - 10, "gui.ancientwarfarevehicle.inventory.player"));
        Button stats = new Button(8, 4, 45, 16, "gui.ancientwarfarevehicle.inventory.stats") {
            @Override
            protected void onPressed() {
                AWMenuTypes.open(player, NetworkHandler.GUI_VEHICLE_STATS, getContainer().vehicle.getId());
            }
        };
        addGuiElement(stats);
        Button pack = new Button(8 + 45 + 4, 4, 45, 16, "gui.ancientwarfarevehicle.inventory.pack") {
            @Override
            protected void onPressed() {
