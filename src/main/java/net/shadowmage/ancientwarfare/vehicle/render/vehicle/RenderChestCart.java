...
public class RenderChestCart extends RenderVehicleBase {
    ...
    public RenderChestCart(EntityRendererProvider.Context renderManager) {
        super(renderManager);
    }

    @Override
    public void renderVehicle(VehicleBase vehicle, double x, double y, double z, float yaw, float tick) {
        model.setHitchAngle(Mth.abs(Trig.getVelocity(vehicle.getDeltaMovement().x, vehicle.getDeltaMovement().y, vehicle.getDeltaMovement().z)) > 0.02f ? 0.f : 25.f);
