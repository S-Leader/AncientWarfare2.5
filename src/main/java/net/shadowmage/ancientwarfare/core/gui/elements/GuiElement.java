...
public abstract class GuiElement {
    ...
    /*
     * 1.20 replacements for the removed FontRenderer draw methods.
     * Text is drawn immediately through the shared buffer source with an identity matrix so
     * it shares the coordinate space (and current model-view transform) of the legacy widget quads.
     */
    public static void drawString(String text, float x, float y, int color) {
        drawString(Minecraft.getInstance().font, text, x, y, color, false);
    }

    public static void drawStringWithShadow(String text, float x, float y, int color) {
        drawString(Minecraft.getInstance().font, text, x, y, color, true);
    }

    public static void drawString(Font font, String text, float x, float y, int color, boolean dropShadow) {
        MultiBufferSource.BufferSource buffer = Minecraft.getInstance().renderBuffers().bufferSource();
        font.drawInBatch(text, x, y, color, dropShadow, new Matrix4f(), buffer, Font.DisplayMode.NORMAL, 0, LightTexture.FULL_BRIGHT);
        buffer.endBatch();
    }

    /*
     * word-wrapped text rendering, equivalent of 1.12 FontRenderer#drawSplitString
     */
    public static void drawSplitString(Font font, String text, int x, int y, int wrapWidth, int color) {
        MultiBufferSource.BufferSource buffer = Minecraft.getInstance().renderBuffers().bufferSource();
        int lineY = y;
        for (FormattedCharSequence line : font.split(FormattedText.of(text), wrapWidth)) {
            font.drawInBatch(line, x, lineY, color, false, new Matrix4f(), buffer, Font.DisplayMode.NORMAL, 0, LightTexture.FULL_BRIGHT);
    ...
    public static void drawSplitString(String text, int x, int y, int wrapWidth, int color) {
        drawSplitString(Minecraft.getInstance().font, text, x, y, wrapWidth, color);
    }
    ...
    public void addTooltip(String text, int maxWidthInChars) {
        // wrap around \n characters
        List<String> textLines = Arrays.asList(I18n.get(text).split("\\\\n"));
        ...
        int width = Minecraft.getInstance().font.width(textLinesWrapped.get(0));
        int height = 0;

        // pre-calculate the width and height (necessary for multiline tooltips)
        for (int i = 0; i < textLinesWrapped.size(); i++) {
            int lineWidth = Minecraft.getInstance().font.width(textLinesWrapped.get(i));
            if (lineWidth > width)
                width = lineWidth;
    ...
    public void setTooltipIfFound(String name) {
        ...
        if (tooltipFound != null)
            addTooltip(I18n.get(tooltipFound));
    }
