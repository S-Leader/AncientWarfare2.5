...
public class GuiResearchBook extends GuiContainerBase {
    ...
    private void addRecipeModeControls() {
        ...
        for (ResearchRecipeBase recipe : AWCraftingManager.getRecipes().stream().sorted(Comparator.comparing(r -> r.getRecipeOutput().getDisplayName().getString()))
        ...
        if (selectedRecipe != null) {
            ...
            boolean canShow = ResearchTracker.INSTANCE.hasPlayerCompleted(mc.level, mc.player.getName().getString(), depResearch);
            int recipeWidth = 3;
            int recipeHeight = 3;
            ...
            if (canShow) {
                ...
                for (int i = 0; i < selectedRecipe.getIngredients().size(); i++) {
                    ...
                    ItemStack displayStack = ResearchIngredientDisplay.getFirstDisplayStack(ingredient);
                    if (!displayStack.isEmpty()) {
                        ingredients.set(i, displayStack);
                    }
                }
    ...
    private Button getRecipeButton(int topLeftX, int topLeftY, ResearchRecipeBase recipe) {
        return new Button(topLeftX, topLeftY, 160, 12, recipe == null ? "guistrings.no_selection" : recipe.getRecipeOutput().getDisplayName().getString());
    }
    ...
    private class RecipeButton extends Button {
        ...
        public RecipeButton(int topLeftX, int topLeftY, ResearchRecipeBase recipe) {
            super(topLeftX, topLeftY, 160, 12, recipe == null ? "guistrings.no_selection" : recipe.getRecipeOutput().getDisplayName().getString());
            this.recipe = recipe;
        }
    ...
    private Button getResearchButton(int topLeftX, int topLeftY, ResearchGoal goal) {
        return new Button(topLeftX, topLeftY, 160, 10, goal == null ? "guistrings.no_selection" : goal.getDescriptionId());
    }
    ...
    private class GoalButton extends Button {
        ...
        public GoalButton(int topLeftX, int topLeftY, ResearchGoal goal) {
            super(topLeftX, topLeftY, 160, 10, goal == null ? "guistrings.no_selection" : goal.getDescriptionId());
            this.goal = goal;
        }
    ...
    private class ResearchSorter implements Comparator<ResearchGoal> {
        ...
        public int compare(ResearchGoal arg0, ResearchGoal arg1) {
            String nameA = I18n.get(arg0.getName());
            String nameB = I18n.get(arg1.getName());
