...
public class NpcSkinManager {
    ...
    public static void loadSkins() {
        if (AWNPCStatics.loadDefaultSkinPack) {
            Path bundledSkins = ModResourceHelper.findModResource(
                    AncientWarfareCore.MOD_ID, "assets", "ancientwarfare", "skin_pack");
            if (bundledSkins != null) {
                loadSkinsFromSource(bundledSkins);
            } else {
                AncientWarfareCore.LOG.error("Unable to locate bundled NPC skins at {}", DEFAULT_SKINS);
            }
        }
        loadSkinsFromSource(new File(SKINS_CONFIG_PATH).toPath());
    }

    @SuppressWarnings("squid:S3725") //can't use toFile().exists() as it's unsupported in ZipPath
    private static void loadSkinsFromSource(Path root) {
        HashMap<String, Set<String>> imageMap = new HashMap<>();

        FileUtils.findFiles(root, skinRoot -> {
            Path fPath = skinRoot.resolve("skin_pack.meta");
            ...
            String relative = skinRoot.relativize(file).toString().replace('\\', '/');
    ...
    private static ResourceLocation loadSkinImage(String imageName, InputStream is) {
        return AncientWarfareNPC.proxy.loadSkinPackImage(imageName, is).orElse(MissingTextureAtlasSprite.getLocation());
    }
