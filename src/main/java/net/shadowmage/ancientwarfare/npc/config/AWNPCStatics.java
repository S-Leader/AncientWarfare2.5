...
import net.shadowmage.ancientwarfare.core.config.legacy.LegacyConfigCategory;
...
public class AWNPCStatics extends ModConfiguration {
    ...
    public static int shieldReactionDelay = 5;
    public static int shieldBowReactionDelay = 30;


    /* ********************************************CLIENT SETTINGS************************************************ */
    public static boolean loadDefaultSkinPack = true;

    public static LegacyProperty renderAI, renderWorkPoints, renderFriendlyNames, renderHostileNames, renderFriendlyHealth, renderHostileHealth, renderTeamColors;

    /* ********************************************FOOD SETTINGS************************************************ */
    private LegacyConfiguration foodConfig;
    ...
    private LegacyConfiguration factionConfig;
    ...
    public void initializeValues() {
        ...
        shieldReactionDelay = Math.max(1, config.get(serverOptions, "shield_reaction_delay", shieldReactionDelay, "Maximum shield reaction delay in ticks for melee attacks.\nDefault=" + shieldReactionDelay).getInt());

        shieldBowReactionDelay = Math.max(1, config.get(serverOptions, "shield_bow_reaction_delay", shieldBowReactionDelay, "Maximum shield reaction delay in ticks for aimed ranged attacks.\nDefault=" + shieldBowReactionDelay).getInt());
    ...
    private void loadFoodValues() {
        ...
        LegacyConfigCategory category = foodConfig.getCategory(foodSettings);
        foodValues = new HashMap<>();
        String name;
        int value;
        for (Entry<String, LegacyProperty> entry : category.entrySet()) {
            name = entry.getKey();
    ...
    public int getFoodValue(ItemStack stack) {
        ...
        if (food != null) {
            ...
            return (nutrition + nutrition * (int) (saturationModifier * 2F)) * foodMultiplier;
        }
        return 0;
    }
