...
public class NpcAIPlayerOwnedTrader extends NpcAI<NpcPlayerOwned> {
    ...
    public boolean canUse() {
        return super.canUse() && orders != null && currentWaypoint != null;
    }
    ...
    private boolean tryWithdrawUpkeep() {
        return npc.getUpkeepPoint()
                .map(upkeepPoint -> WorldTools.getItemHandlerFromTile(npc.level(), upkeepPoint, npc.getUpkeepBlockSide()).map(h -> npc.withdrawFood(h)).orElse(false))
                .orElse(false);
    }
    ...
    private void doDeposit() {
        ItemStack backpack = npc.getMainHandItem();
        if (ItemBackpack.isBackpack(backpack)) {
            IItemHandler inv = backpack.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null).orElse(null);
            BlockPos pos = orders.getRestockData().getDepositPoint();
            //noinspection ConstantConditions
            WorldTools.getItemHandlerFromTile(npc.level(), pos, orders.getRestockData().getDepositSide())
    ...
    private void doWithdraw() {
        ...
        if (ItemBackpack.isBackpack(backpack)) {
            ...
            //noinspection ConstantConditions
            WorldTools.getItemHandlerFromTile(npc.level(), pos, orders.getRestockData().getWithdrawSide())
