...
public class GuiTradeOrder extends GuiContainerBase<ContainerTradeOrder> {
    ...
    private int addRoutePoint(final POTradePoint point, final int index, int startHeight) {
        ...
        if (player.level().hasChunkAt(pos)) {
            blockName.setText(player.level().getBlockState(pos).getBlock().getDescriptionId());
        }
    ...
    private void setupRestockMode() {
        ...
        restockArea.addGuiElement(new Button(70, totalHeight, 55, 12, net.shadowmage.ancientwarfare.core.block.Direction.getTranslationKey(restock.getDepositSide())) {
            @Override
            protected void onPressed() {
                Direction dir = Direction.values()[(restock.getDepositSide().ordinal() + 1) % Direction.values().length];
                setText(net.shadowmage.ancientwarfare.core.block.Direction.getTranslationKey(restock.getDepositSide()));
        ...
        totalHeight += 12;
        restockArea.addGuiElement(new Label(8, totalHeight, "guistrings.side"));
        restockArea.addGuiElement(new Button(70, totalHeight, 55, 12, net.shadowmage.ancientwarfare.core.block.Direction.getTranslationKey(restock.getWithdrawSide())) {
            @Override
            protected void onPressed() {
                Direction dir = Direction.values()[(restock.getWithdrawSide().ordinal() + 1) % Direction.values().length];
                setText(net.shadowmage.ancientwarfare.core.block.Direction.getTranslationKey(restock.getWithdrawSide()));
