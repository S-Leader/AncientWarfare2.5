...
public class GuiNpcInventory extends GuiContainerBase<ContainerNpcInventory> {
    ...
    public void initElements() {
        ...
        nameInput = new Text(75, 8, 95, getContainer().entity.hasCustomName() ? getContainer().entity.getCustomName().getString() : "", this) {
            @Override
        ...
        skinButton = new Button(75, 20, 95, 12, getContainer().skinSettings.getDescription()) {
            ...
            protected void onPressed() {
                Minecraft.getInstance().setScreen(new GuiSkinSelection(GuiNpcInventory.this, getContainer()));
            }
        };
        ...
        if (!AWNPCStatics.repackCreativeOnly || (AWNPCStatics.repackCreativeOnly && player.getAbilities().instabuild)) {
        ...
        if (player.getAbilities().instabuild) {
            button = new Button(buttonX, 96, 75, 12, "guistrings.npc.creative_gui") {
                @Override
                protected void onPressed() {
                    AWMenuTypes.open(player, NetworkHandler.GUI_NPC_CREATIVE, getContainer().entity.getId(), 0, 0);
                }
            };
            addGuiElement(button);
        }
    ...
    public void setupElements() {
        nameInput.setText(getContainer().entity.hasCustomName() ? getContainer().entity.getCustomName().getString() : "");
        if (getContainer().isArcher) {
