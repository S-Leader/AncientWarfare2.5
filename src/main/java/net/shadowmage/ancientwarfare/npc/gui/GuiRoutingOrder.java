...
public class GuiRoutingOrder extends GuiContainerBase<ContainerRoutingOrder> {
    ...
    public void setupElements() {
        ...
        for (RoutePoint point : entries) {
            ...
            label = new Label(8, totalHeight, block == null ? "" : block.getName().getString());
            area.addGuiElement(label);

            label = new Label(120, totalHeight, StringTools.formatPos(pos));
            area.addGuiElement(label);

            labelString = Direction.getTranslationKey(point.getBlockSide());
            ...
            labelString = point.getIgnoreDamage() ? ChatFormatting.RED.toString() + ChatFormatting.STRIKETHROUGH.toString() : "";
            labelString += I18n.get("guistrings.dmg");
            ...
            button.addTooltip(I18n.get("guistrings.dmg.tooltipprefix." + (point.getIgnoreDamage() ? "disabled" : "enabled")) + I18n.get("guistrings.dmg.tooltip"), 28);
            area.addGuiElement(button);

            labelString = point.getIgnoreTag() ? ChatFormatting.RED.toString() + ChatFormatting.STRIKETHROUGH.toString() : "";
            labelString += I18n.get("guistrings.tag");
            ...
            button.addTooltip(I18n.get("guistrings.tag.tooltipprefix." + (point.getIgnoreTag() ? "disabled" : "enabled")) + I18n.get("guistrings.tag.tooltip"), 28);
            area.addGuiElement(button);

            for (int i = 0; i < point.getFilterSize(); i++) {
                final int filterIndex = i;
                slot = new IndexedRoutePointItemSlot(9 + i * 18, totalHeight + 10 + 12 + 2, point.getFilterInSlot(i), this, point, filterIndex) {
                    @Override
                    public void onSlotClicked(ItemStack stack, boolean rightClicked) {
                        onFilterSlotClicked(this, point, filterIndex, stack, rightClicked);
                    }
                };
                slot.setQuantityAdjustable(updatedStack -> {
                    point.setFilter(filterIndex, updatedStack);
                    hasChanged = true;
                });
    ...
    private void onFilterSlotClicked(ItemSlot slot, RoutePoint point, int index, ItemStack stack, boolean rightClicked) {
        if (stack.isEmpty()) {
