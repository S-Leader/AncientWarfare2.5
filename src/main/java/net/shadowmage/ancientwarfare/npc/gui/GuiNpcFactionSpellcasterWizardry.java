...
public class GuiNpcFactionSpellcasterWizardry extends GuiContainerBase<ContainerNpcFactionSpellcasterWizardry> {
    ...
    public void initElements() {
        ...
        for (int i = 0; i < 5; i++) {
            String name = I18n.get("entity.ancientwarfarenpc." + getContainer().entity.getNpcFullType() + "." + (i + 1 + ".name"));
    ...
    private class SpellAddButton extends Button {
        ...
        private SpellAddButton(int topLeftX, int topLeftY, ResourceLocation spell) {
            super(topLeftX, topLeftY, 115, 12, spell.toString());
            this.spell = spell;
        }
    ...
    private class SpellRemoveButton extends Button {
        ...
        private SpellRemoveButton(int topLeftX, int topLeftY, ResourceLocation spell) {
            super(topLeftX, topLeftY, 115, 12, spell.toString());
            this.spell = spell;
        }
    ...
    public void setupElements() {
        ...
        List<ResourceLocation> castableSpells = new ArrayList<>();
		 ...
		 */
        for (ResourceLocation currSpell : castableSpells.stream()
                .filter(spell -> spell.toString().toLowerCase().contains(spellFilterInput.getText().toLowerCase()))
                .sorted(Comparator.comparing(spell -> spell.toString().toLowerCase())).collect(Collectors.toList())) {
        ...
        List<ResourceLocation> assignedSpells = getContainer().getAssignedSpells();

		/*
		Generates an ordered button list of all assigned spells. Clicking on a button will unassign that spell
		 */
        for (ResourceLocation currSpell : assignedSpells.stream()
                .sorted(Comparator.comparing(spell -> spell.toString().toLowerCase())).collect(Collectors.toList())) {
