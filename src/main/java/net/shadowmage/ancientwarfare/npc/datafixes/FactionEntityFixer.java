...
public class FactionEntityFixer implements ILegacyDataFixer {
    ...
    public CompoundTag fixTagCompound(CompoundTag compound) {
        ...
        if (factionFixes.keySet().contains(id)) {
            ...
            compound.putString("id", idFaction.getA());
            compound.putString("factionName", idFaction.getB());
            if (compound.contains("horseAI") && compound.getCompound("horseAI").getBoolean("warHorseKilled")) {
                compound.putBoolean("horseLives", false);
            }
        }
    ...
    @SubscribeEvent
    public void missingMapping(MissingMappingsEvent event) {
        for (MissingMappingsEvent.Mapping<EntityType<?>> entry :
                event.getMappings(ForgeRegistries.Keys.ENTITY_TYPES, AncientWarfareNPC.MOD_ID)) {
            if (factionFixes.containsKey(entry.getKey().toString())) {
                entry.ignore();
            }
        }
    }
