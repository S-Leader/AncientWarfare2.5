...
public class FactionNpcDefault extends NpcDefault {
    ...
    private final Map<String, String> compatibilityData = new HashMap<>();
    ...
    public FactionNpcDefault(Map<String, Double> attributes, int experienceDrop, boolean canSwim, boolean canBreakDoors, Map<Integer, Item> equipment,
        ...
        //1.20: loot tables load from datapacks automatically, no registration required
    }

    private FactionNpcDefault copy() {
        FactionNpcDefault copy = new FactionNpcDefault(new HashMap<>(attributes), experienceDrop, canSwim, canBreakDoors, new HashMap<>(equipment), new HashMap<>(additionalAttributes), enabled, lootTable, heightRange, thinness);
        copy.compatibilityData.putAll(compatibilityData);
        return copy;
    }
    ...
    public FactionNpcDefault setCompatibilityData(String key, String value) {
        return change(def -> def.compatibilityData.put(key, value));
    }
    ...
    public String getCompatibilityData(String key) {
        return compatibilityData.getOrDefault(key, "");
    }
}
