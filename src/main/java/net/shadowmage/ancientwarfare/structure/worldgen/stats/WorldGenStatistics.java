...
public class WorldGenStatistics {
    ...
    public static void addStructureGeneratedInfo(String structureName, Level world, BlockPos pos) {
        if (!collectWorldGenStats) {
            return;
        }
        StructureRecord structureRecord = getStructureRecord(structureName);
        ...
        structureRecord.addBiomeGen(world.getBiome(pos).unwrapKey().map(key -> key.location().toString()).orElse("unknown"));
