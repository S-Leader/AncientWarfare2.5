...
public class GuiSpawnerAdvancedAddEntity extends GuiContainerBase {
    ...
    private void loadData() {
        CompoundTag tag = this.settings.getCustomTag();
        if (tag != null) {
    ...
    private void saveTag() {
        if (!tagInput.isEmpty()) {
            ...
            try {
                CompoundTag base = TagParser.parseTag(tag);
    ...
    public void setupElements() {
        ...
        button = new Button(100, totalHeight, 120, 12, settings.getEntityName()) {
            @Override
            protected void onPressed() {
                Minecraft.getInstance().setScreen(new GuiSpawnerAdvancedEntitySelection(GuiSpawnerAdvancedAddEntity.this, settings));
            }
        };
