...
public class GuiStake extends GuiContainerBase<ContainerStake> {
    ...
    private int addEntityControls(int totalHeight) {
        addGuiElement(new Button(SUBCONTROLS_INDENT, totalHeight, 160, 12, getContainer().getStake().getEntityName().toString()) {
            ...
            protected void onPressed() {
                Minecraft.getInstance().setScreen(new GuiSelectFromList<>(GuiStake.this, getContainer().getStake().getEntityName(), ResourceLocation::toString,
            ...
            private List<ResourceLocation> getEntityNames() {
                return ForgeRegistries.ENTITY_TYPES.getKeys().stream().sorted(Comparator.naturalOrder()).collect(Collectors.toList());
            }
        });
    ...
    protected boolean onGuiCloseRequested() {
        ...
        getContainer().getStake().getWorld().getLightEngine().checkBlock(getContainer().getStake().getPos());
        return true;
    }
}
