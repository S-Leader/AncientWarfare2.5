...
import net.shadowmage.ancientwarfare.structure.template.build.validation.properties.*;
...
public class GuiStructureValidationSettings extends GuiContainerBase {
    ...
    public void setupElements() {
        typeLabel.setText(I18n.get("guistrings.validation_type") + ": " + parent.getContainer().getValidationTypeName());
    ...
    private class PropertyBiomeInput extends Button {
        ...
        protected void onPressed() {
            Minecraft.getInstance().setScreen(new GuiSelectFromList<>(GuiStructureValidationSettings.this, PropertyBiomeInput.this.text, s -> s,
                    () -> ForgeRegistries.BIOMES.getEntries().stream().map(e -> e.getKey().location().toString()).sorted(Comparator.naturalOrder()).collect(() -> {
