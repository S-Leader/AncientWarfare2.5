...
public class GuiLootChestPlacer extends GuiContainerBase<ContainerLootChestPlacer> {
    ...
    private int addSpawnEntityElements(int totalHeight) {
        ...
        addGuiElement(new Button(x, totalHeight, 250, 12, getLootSetting(LootSettings::getEntity).map(ResourceLocation::toString).orElse("")) {
            ...
            protected void onPressed() {
                Minecraft.getInstance().setScreen(new GuiSelectFromList<>(GuiLootChestPlacer.this, text, s -> s,
            ...
            private List<String> getEntityNames() {
                return ForgeRegistries.ENTITY_TYPES.getKeys().stream().map(ResourceLocation::toString).sorted(Comparator.naturalOrder()).collect(Collectors.toList());
            }
        });
        totalHeight += 16;
        ...
        addGuiElement(new Text(x + 60, totalHeight, 190, getLootSetting(s -> s.getEntityNBT().toString()).orElse(""), this) {
            ...
            public void onTextUpdated(String oldText, String newText) {
                try {
                    CompoundTag entityNBT = TagParser.parseTag(newText);
    ...
    private int addSetLootTableElements(int totalHeight) {
        ...
        Button selection = new Button(x, totalHeight, 250, 12, getLootSetting(s -> s.getLootTableName().map(lootTable -> this.shortenCompatLootTables(lootTable.toString())).orElse("")).orElse("")) {
            @Override
            protected void onPressed() {
                Minecraft.getInstance().setScreen(new GuiSelectFromList<>(GuiLootChestPlacer.this,
    ...
    private int addSplashPotionElements(int totalHeight) {
        ...
        Optional<List<MobEffectInstance>> potionEffects = getLootSetting(LootSettings::getEffects);

        if (potionEffects.isPresent()) {
            List<MobEffectInstance> effects = potionEffects.get();
            for (int i = 0; i < effects.size(); i++) {
                ...
                addGuiElement(new Button(x + 230, totalHeight, 20, 12, "-") {
                    ...
                    // remove is used in a button callback so terchnically outside of the loop
                    protected void onPressed() {
            ...
            addGuiElement(new Button(x, totalHeight, 20, 12, "+") {
                @Override
                protected void onPressed() {
                    effects.add(new MobEffectInstance(MobEffects.POISON, 200, 1));
                    refreshGui();
                    setLootSettings(s -> s.setEffects(effects));
                }
            });
    ...
    private static class PotionEffectElement extends Composite {
        private MobEffectInstance potionEffect;

        private PotionEffectElement(int topLeftX, int topLeftY, MobEffectInstance effect, GuiContainerBase parent, Consumer<MobEffectInstance> onEffectUpdated) {
            super(parent, topLeftX, topLeftY, 230, 12);
            potionEffect = effect;

            //noinspection ConstantConditions
            Button selectEffect = new Button(0, 0, 160, 12, ForgeRegistries.MOB_EFFECTS.getKey(potionEffect.getEffect()).toString()) {
                @Override
                protected void onPressed() {
                    Minecraft.getInstance().setScreen(new GuiSelectFromList<>(parent, text, s -> s,
                            PotionEffectElement.this::getEffectNames, s -> {
                        setText(s);
                        //noinspection ConstantConditions
                        potionEffect = new MobEffectInstance(ForgeRegistries.MOB_EFFECTS.getValue(new ResourceLocation(s)), potionEffect.getDuration(), potionEffect.getAmplifier());
            ...
            NumberInput duration = new NumberInput(170, 0, 22, potionEffect.getDuration() / 20f, parent) {
                @Override
                public void onValueUpdated(float value) {
                    potionEffect = new MobEffectInstance(potionEffect.getEffect(), (int) value * 20, potionEffect.getAmplifier());
                    onEffectUpdated.accept(potionEffect);
                }
            };
            ...
            NumberInput amplifier = new NumberInput(200, 0, 15, potionEffect.getAmplifier(), parent) {
                @Override
                public void onValueUpdated(float value) {
                    potionEffect = new MobEffectInstance(potionEffect.getEffect(), potionEffect.getDuration(), (int) value);
                    onEffectUpdated.accept(potionEffect);
                }
            };
        ...
        private List<String> getEffectNames() {
            return ForgeRegistries.MOB_EFFECTS.getKeys().stream().map(ResourceLocation::toString).sorted(Comparator.naturalOrder()).collect(Collectors.toList());
        }
    }
    ...
    private int addPlaceBlockElements(int totalHeight) {
        ...
        for (ItemLootChestPlacer.LootContainerInfo container : ItemLootChestPlacer.getLootContainers().values()) {
            ...
            button.addTooltip(containerStack.getHoverName().getString()); // tooltip for hovering loot items
