...
public class GuiLootBasket extends GuiContainerBase<ContainerLootBasket> {
    ...
    public void initElements() {
        ...
        if (getContainer().getItemHandler().map(inventory -> inventory.getSlots() > 27).orElse(false)) {
            addGuiElement(new Label(8, 128, player.getInventory().getDisplayName().getString()).setColor(4210752));
        } else {
            addGuiElement(new Label(8, 74, player.getInventory().getDisplayName().getString()).setColor(4210752));
        }
    }
