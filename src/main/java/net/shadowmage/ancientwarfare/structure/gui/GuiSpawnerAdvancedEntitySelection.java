...
public class GuiSpawnerAdvancedEntitySelection extends GuiContainerBase {
    ...
    public void setupElements() {
        ...
        List<ResourceLocation> entities = ForgeRegistries.ENTITY_TYPES.getKeys().stream().filter(rl -> {
        ...
        }).sorted(Comparator.comparing(registryName -> I18n.get(EntityTools.getUnlocName(registryName)))).collect(Collectors.toList());
        int totalHeight = 8;
        Button button;
        for (ResourceLocation registryName : entities) {
            button = new Button(8, totalHeight, 256 - 8 - 16, 12, I18n.get(EntityTools.getUnlocName(registryName))) {
                @Override
                protected void onPressed() {
