...
public class GuiStructureBiomeSelection extends GuiContainerBase {
    ...
    public void initElements() {
        ...
        addGuiElement(new Label(8, 22, I18n.get("guistrings.scanner.biome_groups") + ":"));
        ...
        addGuiElement(new Label(8, 138, I18n.get("guistrings.scanner.biomes") + ":"));
    ...
    private void refreshBiomeList() {
        ...
        for (Biome biome : ForgeRegistries.BIOMES) {
            ...
            if (name.contains(biomeSearchBox.getText()) || getBiomeName(biome).contains(biomeSearchBox.getText())) {
    ...
    private class BiomeCheck extends Checkbox {
        ...
        public BiomeCheck(int topLeftY, Biome biome) {
            //noinspection ConstantConditions
            super(8, topLeftY, 16, 16, String.format("%s (%s)", getBiomeName(biome), ForgeRegistries.BIOMES.getKey(biome).toString()));
            //noinspection ConstantConditions
            this.name = ForgeRegistries.BIOMES.getKey(biome).toString();
        }
    }

    private static String getBiomeName(Biome biome) {
        //noinspection ConstantConditions
        return I18n.get(Util.makeDescriptionId("biome", ForgeRegistries.BIOMES.getKey(biome)));
    }
}
