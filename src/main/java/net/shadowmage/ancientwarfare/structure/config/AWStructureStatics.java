...
public class AWStructureStatics extends ModConfiguration {
    ...
    public static int constructionFloodFillMaxDistance = 40;
    ...
    protected void initializeValues() {
        ...
        constructionFloodFillMaxDistance = Math.max(1, config.get(serverOptions, "construction_flood_fill_max_distance", constructionFloodFillMaxDistance, "Maximum block distance searched by construction-tool lake and layer fill modes.\nDefault=" + constructionFloodFillMaxDistance).getInt());
    ...
    public static void logSkippableBlocksCoveredByMaterial() {
        skippableWorldGenBlocks.stream().filter(b -> getBlock(b).isPresent() && isSkippableMaterial(LegacyMaterial.of(getBlock(b).get().defaultBlockState())))
    ...
    public static boolean isValidTargetBlock(BlockState state) {
        try {
            //noinspection ConstantConditions
            return isValidTargetMaterial(LegacyMaterial.of(state)) || worldGenTargetBlocks.contains(String.valueOf(ForgeRegistries.BLOCKS.getKey(state.getBlock())));
    ...
    public static boolean isValidTargetMaterial(LegacyMaterial material) {
        return material == LegacyMaterial.GRASS || material == LegacyMaterial.GROUND || material == LegacyMaterial.ROCK || material == LegacyMaterial.SNOW || material == LegacyMaterial.ICE
                || material == LegacyMaterial.PACKED_ICE || material == LegacyMaterial.SAND || material == LegacyMaterial.WATER;
    }

    public static boolean isSkippable(BlockState state) {
        try {
            //noinspection ConstantConditions
            return isSkippableMaterial(LegacyMaterial.of(state)) || skippableWorldGenBlocks.contains(String.valueOf(ForgeRegistries.BLOCKS.getKey(state.getBlock())));
        } catch (NullPointerException e) {
            // Error handling
    ...
    public static boolean isSkippableMaterial(LegacyMaterial material) {
        return material == LegacyMaterial.AIR || material == LegacyMaterial.PLANTS || material == LegacyMaterial.VINE || material == LegacyMaterial.LEAVES || material == LegacyMaterial.WOOD
                || material == LegacyMaterial.GOURD || material == LegacyMaterial.CACTUS;
    }

    public static boolean shouldSkipScan(Block block) {
        ResourceLocation registryName = ForgeRegistries.BLOCKS.getKey(block);
        return registryName != null && scannerSkippedBlocks.contains(registryName.toString());
    }
