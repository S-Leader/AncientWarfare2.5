...
class TownGeneratorStructures {
    ...
    private static void generateLamps(Level world, TownPartBlock block, StructureTemplate lamp, List<BlockPos> doors) {
    ...
    private static void generateLamp(Level world, StructureTemplate template, List<BlockPos> doors, int x, int y, int z, Direction streetSide) {
    ...
    private static boolean generateStructureForPlot(TownGenerator gen, TownPartPlot plot, StructureTemplate template, boolean centerLength) {
        ...
        net.minecraft.core.Direction face = net.minecraft.core.Direction.from2DDataValue(gen.rng.nextInt(4));//select random face
        for (int i = 0; i < 4; i++) {//and then iterate until a valid face is found
            face = face.getClockWise();
            if (plot.roadBorders[face.get2DDataValue()]) {
                break;
            }
        }
        face = face.getOpposite();//reverse face from road edge...
        int width = face.getAxis() == net.minecraft.core.Direction.Axis.Z ? template.getSize().getX() : template.getSize().getZ();
        int length = face.getAxis() == net.minecraft.core.Direction.Axis.Z ? template.getSize().getZ() : template.getSize().getX();
        if (face == net.minecraft.core.Direction.SOUTH || face == net.minecraft.core.Direction.NORTH) {
            width += expansion;
     ...
     */
    private static void generateStructure(TownGenerator gen, TownPartPlot plot, StructureTemplate template, net.minecraft.core.Direction face, int width, int length, boolean center) {
        ...
        if (center) {
            ...
            if (face.getAxis() == net.minecraft.core.Direction.Axis.Z) {
                wAdj = extraWidth / 2;
            } else {
                wAdj = face == net.minecraft.core.Direction.WEST ? extraWidth : 0;
            }
            if (face.getAxis() == net.minecraft.core.Direction.Axis.X) {
                lAdj = extraLength / 2;
            } else {
                lAdj = face == net.minecraft.core.Direction.NORTH ? extraLength : 0;
            }
        }
