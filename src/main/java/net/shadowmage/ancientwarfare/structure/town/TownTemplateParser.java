...
public class TownTemplateParser {
    ...
    private static final Map<String, BiConsumer<String, TownTemplate>> headerVariableSetters = new ImmutableMap.Builder<String, BiConsumer<String, TownTemplate>>()
            ...
            .put("lamp", (value, template) -> {
                if (!value.isBlank() && !value.equals("0")) {
                    template.setLamp(new TownStructureEntry(value, 1));
                }
            })
            .put("biomereplacement", TownTemplateParser::setBiomeReplacement)
    ...
    private static boolean parseHeader(Iterator<String> it, TownTemplate template) {
        ...
        if (template.getRoadFillBlocks().isEmpty()) {
            template.addRoadFillBlock(Blocks.GRAVEL.defaultBlockState());
        }
        return true;
    }
    ...
    private static List<String> parseBiomeList(String line) {
        ...
        for (String bit : bits) {
            names.add(LegacyBiomeNames.remap(bit));
        }
        return names;
    }

    private static void setBiomeReplacement(String value, TownTemplate template) {
        ResourceLocation replacement = ResourceLocation.tryParse(LegacyBiomeNames.remap(value));
        if (replacement != null && !replacement.getPath().isEmpty()) {
            template.setBiomeReplacement(replacement);
        }
    }
