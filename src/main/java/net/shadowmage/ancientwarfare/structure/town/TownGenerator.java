...
public class TownGenerator {
    ...
    TownGenerator(Level world, TownBoundingArea area, TownTemplate template) {
        this.world = world;
    ...
    private void changeBiome(StructureBB bb) {
        template.getBiomeReplacement().ifPresent(biomeRegistryName -> {
            ...
            BlockPos.betweenClosed(minPos, maxPos).forEach(pos -> WorldTools.changeBiome(world, pos.immutable(), replacementBiome));
        });
    }
    ...
    private void spawnVillager(int minX, int y, int minZ) {
        ...
        if (world instanceof ServerLevel serverLevel) {
            villager.finalizeSpawn(serverLevel, serverLevel.getCurrentDifficultyAt(pos), MobSpawnType.STRUCTURE, null, null);
        }
        for (int i = 0; i < 10; i++) {
            ...
            if (world.isEmptyBlock(pos) && world.isEmptyBlock(pos.above())
                    && world.getBlockState(pos.below()).isFaceSturdy(world, pos.below(), net.minecraft.core.Direction.UP)) {
                villager.setPos(x + 0.5d, y, z + 0.5d);
