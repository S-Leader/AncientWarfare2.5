...
public final class TownTemplate {
    ...
    private List<BlockState> roadFillBlocks = new ArrayList<>();
    ...
    public final void validateStructureEntries() {
        ...
        if (e.isPresent() && !StructureTemplateManager.getTemplate(e.get().templateName).isPresent()) {
            AncientWarfareStructure.LOG.info("Skipping unavailable optional lamp template: {}", e.get().templateName);
        }
    ...
    private void validateStructureList(Collection<TownStructureEntry> entries) {
        ...
        while (it.hasNext() && (e = it.next()) != null) {
            if (!StructureTemplateManager.getTemplate(e.templateName).isPresent()) {
                AncientWarfareStructure.LOG.info("Skipping unavailable optional structure template: {} for town: {}", e.templateName, townTypeName);
                it.remove();
            }
        }
    }
    ...
    public List<BlockState> getRoadFillBlocks() {
        return roadFillBlocks;
    }

    public void addRoadFillBlock(BlockState state) {
