...
public class RotateBoundingBox extends AABB {
    private final static float TO_RAD = (float) Math.PI / 180F;
    private final Direction facing;
    //From vertical axis
    private float angle;

    public RotateBoundingBox(Direction face, BlockPos min, BlockPos max) {
        this(face, min.getX(), min.getY(), min.getZ(), max.getX(), max.getY(), max.getZ());
    }

    private RotateBoundingBox(Direction face, double minX, double minY, double minZ, double maxX, double maxY, double maxZ) {
        super(minX, minY, minZ, maxX, maxY, maxZ);
        this.facing = face;
    }

    @Override
    public boolean intersects(AABB mask) {
        if (mask.maxY > minY) {
            double height = Mth.cos(angle * TO_RAD) * (maxY - minY);
            if (mask.minY < height + minY) {
                if (facing.getAxis() == Direction.Axis.Z) {//z
                ...
                double length = Mth.sin(angle * TO_RAD) * (maxY - minY + 1);
